...
function UserRegister() {
    ...
    const [users, setFarmers] = useState(
        {
            name: "",
            email: "",
            phoneNo: ""
        });
    const [address, setUsersAddress] = useState(
        {
            doorNo: "",
            place: "",
            city: "",
    ...
    const handleSubmit = async (event) => {
        ...
        const newusers = { ...users, address };

        setLoading(true)
        const data = await UserService.create(newusers)
            .then(response => {
    ...
    const validate = (values) => {
        ...
        else if (!emailFormat.test(users.email)) {
            errors.email = "Should enter valid email"
        }
        if (!values.phoneNo) {
            errors.phoneNo = "Phone number is required"
        } else if (!phoneFormat.test(users.phoneNo)) {
                                    ...
                                    <div className="card-body p-5">
                                        ...
                                        <form onSubmit={handleSubmit}>
                                            <div className="form-group">
                                                <label >Name:</label>
                                                <input type="text" className="form-control" placeholder="Enter name" name="name" value={users.name} onChange={changeHandle} required />
                                            </div>
                                            <p className='text-danger'>{formEror.name}</p>

                                            <div className="form-group">
                                                <label >Email:</label>
                                                <input type="email" className="form-control" placeholder="Enter email" name="email" value={users.email} onChange={changeHandle} pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" title="Email should be in valid format.(ex: example@gmail.com)" required />
                                            </div>
                                            <p className='text-danger'>{formEror.email}</p>
                                            <div className="form-group">
                                                <label >Phone Number:</label>
                                                <input type="text" className="form-control" placeholder="Enter phone number" name="phoneNo" pattern="^\d{10}$" title="Phone number should be in 10 digits without alphabets" value={users.phoneNo} onChange={changeHandle} required />
                                            </div>
